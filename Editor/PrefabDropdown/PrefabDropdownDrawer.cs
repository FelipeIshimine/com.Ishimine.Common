...
public class PrefabDropdownDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        float dropdownWidth = position.width - labelWidth - EditorGUIUtility.singleLineHeight; // Subtract a button's width

        // Split the position into label and dropdown portions
        Rect labelRect = new Rect(position.x, position.y, labelWidth, position.height);
        Rect dropdownRect = new Rect(position.x + labelWidth, position.y, dropdownWidth, position.height);
        Rect buttonRect = new Rect(position.x + labelWidth + dropdownWidth, position.y, EditorGUIUtility.singleLineHeight, position.height);
