...
/*
    ...
    [MenuItem("GameObject/TakePrefabSnapshot", false, 10)]
    public static void TakePrefabSnapshot()
    {
        var snapCamera = MakeSnapshotCamera(Selection.activeGameObject.layer);
        var texture = snapCamera.TakeObjectSnapshot(Selection.activeGameObject);
        SavePNG(texture,  $"Snap_{Selection.activeGameObject.gameObject}", "Assets/");
        DestroyImmediate(snapCamera.gameObject);
    }
    
    
    [MenuItem("GameObject/TakePrefabSnapshot", true, 10)]
    public static bool TakePrefabSnapshotValidator()
    {
