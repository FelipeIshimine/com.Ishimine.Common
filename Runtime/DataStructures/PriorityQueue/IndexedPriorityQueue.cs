...
public sealed class IndexedPriorityQueue<T> where T : IComparable
{
    ...
    public T this[int index]
    {
        get
        {
            Assert.IsTrue( index < m_objects.Length && index >= 0, $"IndexedPriorityQueue.[]: Index '{index}' out of range");
    ...
    private void SortUpward( int index ) => SortHeapUpward( m_heapInverse[index] );
 
    private void SortDownward( int index ) => SortHeapDownward( m_heapInverse[index] );
    ...
    private void Swap( int i, int j )
    {
        ...
        (m_heap[i], m_heap[j]) = (m_heap[j], m_heap[i]);
    ...
    private static int Parent(int heapIndex ) => ( heapIndex / 2 );
    private static int FirstChild(int heapIndex) => ( heapIndex * 2 );
 
    private int SecondChild(int heapIndex) => ( heapIndex * 2 + 1 );
    #endregion // private
}
